...
class GomokuState(object):
    ...
    def __repr__(self):
        ...
        return 'To play: {}({})\n{}'.format(six.u(self.color), six.u(gomoku_util.color_shape[gomoku_util.color_dict[self.color]]), self.board.__repr__())
...
class GomokuEnv(gym.Env):
    ...
    def __init__(self, player_color, opponent, board_size, opponent_policy=None):
        """
        Args:
    ...
    def _reset_opponent(self, board):
        if self.opponent == 'random':
            ...
            opponent_policy_module = __import__(self.opponent, globals(), locals(), ['policy'], 0)
            self.opponent_policy = opponent_policy_module.policy(self.np_random)
